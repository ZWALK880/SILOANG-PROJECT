...
import view.FiturForgotPassword;
...
public class FiturLogin extends javax.swing.JFrame {
    ...
    private void lb_forgotPasswordMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lb_forgotPasswordMouseClicked
        this.dispose();
        FiturForgotPassword forgotPassword = new FiturForgotPassword();
    ...
    private String[] checkLogin(String email, String password) {
        if (conn != null) {
            try {
                String sql = "SELECT id_user, level, nama FROM user WHERE email=? AND password=?";
